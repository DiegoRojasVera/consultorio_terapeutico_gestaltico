...
class DatosClientes extends StatefulWidget {
  const DatosClientes({super.key});

  @override
  _DatosClientesState createState() => _DatosClientesState();
}

// pedimos el correo para selecciona solo un usuario
class _DatosClientesState extends State<DatosClientes> {
  List? data;
  User? user;
  bool loading = true;
...
//donde se lista por lo que se elige
  Future<List> getData() async {
    ...
    txtemailController.text = user!.email ?? '';
    final response1 =
        await http.get(Uri.parse("http://34.171.207.241/proyecto1/api/userlist"));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Listados de Clientes"),
        ...
        return Container(
          ...
          child: GestureDetector(
            ...
            onTap: () => Navigator.of(context).push(
              MaterialPageRoute(
                  builder: (BuildContext context) => DetailClient(
                        list: list,
                        index: i,
                      )),
            ),
